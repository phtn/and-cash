...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!category || !method) {
      return NextResponse.json({ success: false, message: 'Category and method are required' }, { status: 400 })
    }
    ...
    switch (category) {
      ...
      default:
        return NextResponse.json({ success: false, message: `Unknown category: ${category}` }, { status: 400 })
    }
